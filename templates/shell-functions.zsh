...
_claude_code_bridge_launch() {
  ...
  if nc -z localhost "$port" 2>/dev/null && [ -z "${CLAUDE_BRIDGE_PROXY_PORT:-}" ]; then
    ...
    while nc -z localhost "$port" 2>/dev/null && (( port < max_port )); do
      (( port++ ))
    done
  ...
  if ! nc -z localhost "$port" 2>/dev/null; then
    ...
    # Use start-proxy.py to patch requests (strip unsupported params,
    # fix schemas, remove remote MCP tools) before they reach LiteLLM.
